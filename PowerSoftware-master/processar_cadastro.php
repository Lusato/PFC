...
$hash3= crypt($resposta, '$2a$' . $custo  . $salt);
 ...
 $consu = $conexao -> query("select * from user where email= '$email'");
  ...
  if ($conexao == false) {
			...
			$comando = mysqli_query($conexao, "INSERT INTO user (nome, email, senha,pergunta_secreta, resposta_secreta,tipo_user_fk, atividade_user_fk) VALUES ('$nome','$email','$hash', '$hash2', '$hash3', 2,1)");
            if ($comando == false) {
                echo ("erro ao enviar comando para o banco de dados.");
                $erro = mysqli_error($conexao);
                echo($erro);
